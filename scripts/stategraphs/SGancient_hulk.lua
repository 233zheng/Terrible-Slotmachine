...
local function launchprojectile(inst, dir)
    ...
    local offset = nil
        ...
        offset = FindWalkableOffset(pt, theta, 6 + math.random()*8, 12, true) --12
...
local function ShakeIfClose(inst)
    ...
    if player then
        ShakeAllCameras(CAMERASHAKE.FULL, 0.7, 0.02, 3,inst, SHAKE_DIST)
    end
end
...
local function ShakeIfClose_Footstep(inst)
    local player = GetClosestInstWithTag("player", inst, SHAKE_DIST)
    if player then
        ShakeAllCameras(CAMERASHAKE.FULL, 0.35, 0.02, 1.25,inst, SHAKE_DIST)
    end
end
