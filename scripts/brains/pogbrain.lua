...
local function GetWanderDistFn(inst)
    if TheWorld.state and not TheWorld.state.isday then
...
local function harassPlayer(inst)
    ...
    if item and distsq(p_pt, m_pt) < FOLLOWPLAYER_DIST * FOLLOWPLAYER_DIST and not (target and target.components.driver and target.components.driver:GetIsDriving()) then
        return target
    end
end
