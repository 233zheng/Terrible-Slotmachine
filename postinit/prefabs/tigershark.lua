...
local function FindSharkHome(inst)
    if not inst.sharkHome then
        ...
        --else
        --    inst.sharkHome = GetClosestInstWithTag("sharkhome", inst, 10000) --if theres no tigersharker component then the world likly doesnt have a sharkhome at all, this causes extreme lag when spawned in worlds other than the default islands one -Half
        end
    end
